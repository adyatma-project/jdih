...
class Backend extends CI_Controller {
	public function index()
	{
		if($this->session->userdata('logged_in')=="")
		{
			...
			{
				$this->load->view('backend/login',$d);
			}
			else
			{
		...
		else if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			//$this->session->set_userdata("online",$set_detail->online);
	...
	public function save_pass()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="administrator")
		{
			...
			else
			{
				...
				$login['password'] = md5($pass_lama.'jdihdonggala@xxxaseww21%^&^$#');
				$cek = $this->db->get_where('tbl_user_login', $login);
				if($cek->num_rows()>0)
				{
					if($pass_baru==$ulangi_pass_baru)
					{
						$upd['password'] = md5($pass_baru.'jdihdonggala@xxxaseww21%^&^$#');
						$this->db->update("tbl_user_login",$upd,$id);
	...
	function logout()
	{
		...
		$upd['online'] ="0";
		$this->db->update("tbl_user_login",$upd,$dt);
